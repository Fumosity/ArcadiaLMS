...
// Checks and adds a new entry or increments a current entry to the 'genres'
export const checkAndAddGenre = async (genreData) => {
  try {
    const { data, error } = await supabase.from("genres").select("genreID").eq("genre", genreData.genre).single()
...
// Adds a new record to the genre table
// Modified to return a result instead of handling navigation
