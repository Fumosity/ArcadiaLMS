...
const BookRequests = (props) => {
  ...
  useEffect(() => {
    const fetchReservations = async () => {
      try {
        setIsLoading(true)
        const { data, error } = await supabase.from("book_reservation").select(`
          bookResID, date, status, details, 
          user_accounts:userID (userID, userFName, userLName), 
          book:titleID (titleID, title)
        `)
  ...
  const handleUserClick = (user) => {
    console.log("userid", user.userID, "user", user, user)
            ...
            {isLoading ? (
              ...
              displayedReservations.map((reservation, index) => (
                <tr
                  ...
                  onClick={() => props.onReservationSelect && props.onReservationSelect(reservation)}
                >
                  ...
                  <td
                    ...
                    onClick={(e) => {
                      ...
                      handleUserClick(reservation.user_accounts)
                    }}
                  >
                  ...
                  <td
                    ...
                    onClick={(e) => {
                      e.stopPropagation() // Prevent row click
                      handleBookClick(reservation.book)
                    }}
                  >
                  ...
                  <td
                    className={`flex items-center justify-center mt-2 mb-2 text-sm font-medium rounded-full px-1 py-2 ${getStatusColor(reservation.status)}`}
                  >
                    {reservation.status}
                  </td>
                </tr>
