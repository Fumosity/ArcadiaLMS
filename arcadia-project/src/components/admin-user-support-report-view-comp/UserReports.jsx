...
const UserReports = () => {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        setIsLoading(true)
        const { data, error } = await supabase.from("report_ticket").select(`
  ...
  const filteredData = sortedData.filter((reports) => {
    const matchesType = typeFilter === "All" || reports.type === typeFilter
              ...
              displayedReports.map((report, index) => (
                <tr key={index} className="hover:bg-light-gray cursor-pointer">
                  ...
                  <td className="px-4 py-2 text-sm text-gray-900 text-center w-3/12">
                    <button onClick={() => handleUserClick(report)} className="text-sm text-arcadia-red font-semibold hover:underline">
