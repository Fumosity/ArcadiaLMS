...
export default function OperatingHoursForm({ onCancel, onSuccess }) {
  ...
  useEffect(() => {
    async function fetchHours() {
      try {
        const { data, error } = await supabase.from("operating_hours").select("*").order("dayOrder")
  ...
  useEffect(() => {
    async function fetchHolidayHours() {
      try {
        const { data, error } = await supabase.from("holiday_hours").select("*").order("date")
  ...
  const handleHolidayHourChange = (holidayID, field, value) => {
    setHolidayHours((prev) => prev.map((h) => (h.holidayId === holidayID ? { ...h, [field]: value } : h)))
  }
    ...
    try {
      ...
      for (const holidayHour of holidayHours) {
        ...
        if (existingData) {
          ...
          // Insert new record
          const { error } = await supabase.from("holiday_hours").insert({
            holidayId: holidayHour.holidayId,
            date: holidayHour.date instanceof Date ? holidayHour.date.toISOString().split("T")[0] : holidayHour.date,
            holidayName: holidayHour.holidayName,
            opensAt: holidayHour.isClosed ? null : holidayHour.opensAt,
            closesAt: holidayHour.isClosed ? null : holidayHour.closesAt,
            isClosed: holidayHour.isClosed,
          })
        ...
        {showHolidays && (
          <div className="space-y-4 mt-4">
            <p className="text-sm text-gray-500">
              Set special operating hours for Philippine holidays. If no special hours are set, regular hours will
              apply.
            </p>
                        ...
                        <td className="px-4 py-2 text-right">
                          {hasHours ? (
                            <button
                              ...
                              className="text-arcadia-red hover:underline text-xs"
                            >
                              Cancel
                            </button>
                          ) : (
