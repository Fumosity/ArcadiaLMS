...
function PenaltyReports({ exportData }) {
  ...
  const exportAsXLSX = () => {
    ...
    if (!exportData || !exportData.bkhistoryData || !exportData.damageFinesData) {
      console.error("Export failed: No data available to export")
    ...
    if (exportData.bkhistoryData.length === 0 && exportData.damageFinesData.length === 0) {
      console.error("Export failed: No records found to export")
    ...
    ExcellentExport.convert({ anchor: link, filename: "PenaltyReports", format: "xlsx" }, [
  ...
  return (
    <div className="bg-white p-4 rounded-lg border-grey border">
      ...
      <a id="xlsxDownload" style={{ display: "none" }} href="/">
        Download
      </a>
    </div>
  )
