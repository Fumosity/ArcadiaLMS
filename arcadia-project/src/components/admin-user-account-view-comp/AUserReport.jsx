import { useUserReport } from "../../backend/AUserReportBackend"
import { useEffect } from "react"

const AUserReport = ({ user }) => {
    const {
        sortOrder,
        setSortOrder,
        entriesPerPage,
    ...
    useEffect(() => {
        console.log("AUserReport received user data:", user)
    }, [user])
                        ...
                        {paginatedData.length > 0 ? (
                            paginatedData.map((report, index) => (
                                <tr key={index} className="hover:bg-gray-100 cursor-pointer">
                                    ...
                                    <td className="px-4 py-3 text-sm text-gray-900 text-center">
                                        <span
                                            className={`inline-block font-medium rounded-full px-3 py-1 ${report.statusColor}`}
                                        >
                                            {report.status}
                                        </span>
                                    </td>
